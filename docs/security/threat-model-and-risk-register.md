...
Existing controls include installer Upgrade, Repair, and failed-upgrade Rollback modes; service-aware upgrades; configuration/key preservation; verified pre-upgrade encrypted database backups; checksummed previous-application archives; durable failed-upgrade recovery records; assisted restoration of the previous application and pre-upgrade database together; post-rollback service and health validation; encrypted backup and recovery tooling; ordered versioned database migrations; a persistent migration ledger; per-migration savepoint rollback; and migration tests covering legacy schema upgrades, current schemas, idempotency, and failure handling.

Residual risk remains because full cross-release upgrade and rollback validation across supported release combinations is still required. Interrupted upgrades, interrupted rollbacks, service failures after database activation, and recovery behavior across older published application/database pairs require broader validation.
...
| R-003 | Database or application state becomes corrupted during upgrade or rollback | Medium | High | Upgrade/repair/rollback modes; service-aware upgrade and rollback; configuration/key preservation; verified pre-upgrade database backups; checksummed previous-application archives; durable recovery records; failed-application preservation; staged database recovery activation; post-rollback service and health validation; ordered versioned migrations; persistent migration ledger; per-migration savepoint rollback; legacy-schema and idempotency tests | Cross-release upgrade and rollback validation remains incomplete, especially for interrupted operations and failures after database activation | Add supported-release upgrade/rollback matrix testing, interrupted-operation fault injection, and recovery drills across release boundaries | High |
| R-004 | Release artifact is tampered with, carries incorrect licensing material, or cannot be tied confidently to reviewed source | Medium | High | Versioned artifacts; SHA-256 generation/verification procedures; selected vendor download checksums; version-based repository licensing documentation | Windows installer is not yet formally code-signed; provenance, checksum publication, and packaged-license verification remain incomplete | Add signing guidance/implementation, published checksums, source-tag verification, release provenance procedure, and tests that verify applicable license material is included in release artifacts | High |
...
| R-023 | Published documentation diverges from actual supported deployment, security, recovery, or licensing behavior | Medium | Medium | Structured public documentation, command references, authoritative root license notice, and dedicated licensing guide | Rapid security, installer, recovery, or licensing changes can make documentation stale | Make documentation and license-boundary review part of the release checklist and final release-candidate validation | Medium |
...
- **Fault injection and fail-closed testing:** R-009, R-018
- **Cross-version recovery validation:** R-008
- **Cross-release upgrade and recovery validation:** R-003, R-008
