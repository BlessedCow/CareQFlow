...
| --- | --- | --- | --- | --- | --- | --- | --- |
| R-001 | Production encryption key loss makes protected data unrecoverable | Medium | High | Production validates required keys; backup key is separate from field key | No complete formal key custody, backup, rotation, and recovery lifecycle | Implement and document field, SQLCipher, and backup key lifecycle and tested recovery procedures | Critical |
| R-002 | Production encryption keys are disclosed through host compromise or operator handling | Medium | High | Restricted production directories; secret validation; separate field/backup keys; sensitive-data documentation | Active keys remain accessible to the running application and host administrators | Define secure key custody, ACL verification, rotation, exposure response, and deployment review procedures | Critical |
...
| R-008 | Broken, corrupt, or malicious backup is activated and causes data loss or compromise | Medium | High | Backup verification; encrypted backups; path validation; restore staging; controlled recovery activation | Comprehensive restore fault-injection and cross-version recovery validation remain incomplete | Add failure tests for corrupt backups, wrong keys, incomplete restores, permission failures, and recovery activation | High |
